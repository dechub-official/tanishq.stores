...
const getUpdates=async()=>{
  ...
  if((mno&&mno.match(phoneno)))
        {
            setError(false)
            const data=post("/storeUserDetails",{"mobileNo":mno})
    ...
    const loadData = async () => {
        if (id != "nearbysearch") {
            try {
                setPosition([])
                const data = await get(`/details?storeCode=&city=${id.replace("-", " ")}`)
    ...
    return <>
        ...
        {error&&<div class="shadow-md p-4 bg-white flex fixed top-0 right-0 z-10 flex-row rounded-lg">
      <div class="bg-red-500 inline-block rounded-lg p-1 mr-1"></div>
      {/* <b class="p-1">Alert Title</b> */}
      <p class="p-1">Mobile Number Not Correct</p>
      <a class="h-5 w-5 text-gray-500 cursor-pointer inline-block p-1" onClick={()=>setError(false)}>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
        </svg>
      </a>
    </div>}
     {  success&& <div class="shadow  fixed z-10 top-1 transition-all duration-200 right-0" role="alert">
  <div class="flex">
    <div class="bg-[#832729] w-16 text-center p-2">
    ...
    <div class="bg-white flex relative border-r-4 border-[#832729] w-full p-4">
      <div>
        <p class="text-gray-600 font-bold">Number Added Successfuly</p>
        {/* <p class="text-gray-600 text-sm">Your message thekjsdajl</p> */}
      </div>
      <a class="h-5 w-5 text-gray-500 absolute top-0 right-1 cursor-pointer inline-block p-1" onClick={()=>setSuccess(false)}>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
        </svg>
      </a>
                    ...
                    {data.map((data, i) => {
                        return <div key={i} className="box bg-[#F2F2F2] w-[100%]  my-2 px-2 md:px-8 p-4 rounded-[22px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]">
                            ...
                            <div className="flex justify-items-start ">
                                <div className="w-[50%]">
                                    <Button name="Get Directions" iconstyle={"md:w-[17px]"} style={'text-white bg-[#832729] font-500 text-[10px] px-2'} onclick={() =>{
                                         ReactGA.event({
                                            category:data.storeLocationLink ,
                                            value:Clicks.getDir+1,
                                            action:"Get Directions", });
                                            setClicks({...Clicks,getDir:Clicks.getDir+1})
                                        BookAppointment(data.storeLocationLink)}} icon={map} />
                                </div>
                                <div className="w-[60%]">
                                    <Button name="Book an Appointment" iconstyle={"w-[15px] mr-1"} style={'text-white bg-[#832729] font-500 text-[10px] px-1'} onclick={() =>{
                                        ReactGA.event({
                                            category: "Book An Appointment",
                                            value:Clicks.Bookan+1,
                                            action: "Book An Appointment", });
                                            setClicks({...Clicks,Bookan:Clicks.Bookan+1})
                                        BookAppointment("https://www.tanishq.co.in/book-an-appointment")}} icon={calender} />
                                </div>
                            </div>
                        </div>
                    })}
...
<Heading heading={<p className="text-[20px]">Kakatiya Collections <br /> <span className="text-[18px]"> Exclusively Available at {data.length} Stores</span></p>}/>
            ...
            <div className="flex flex-wrap justify-around items-start w-full">
                ...
                <div className="my-4  w-[88%] flex flex-wrap justify-around items-center border-[1px] border-[#CBC9C9;] rounded-[17px]  shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] py-5 px-4 md:px-6 md:max-h-[900px] max-h-[500px] overflow-y-scroll">
                    ...
                    {data.map((data, i) => {
                        return <>
                            {i == 1 && <div key={i} className="box bg-[#F2F2F2] md:w-[45%] min-h-[300px]  my-4 px-2 md:px-8 p-4 rounded-[22px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]">
                                ...
                                <div className="flex justify-center items-center flex-col">
                                    <h2 className=' text-center text-red-900 text-lg  font-medium md:w-[80%]'>Subscribe to get updates on best offers, gold rates & other benefits from Tanishq.</h2>
                                    <input type="number" min={"10"} onChange={(e)=>setmno(e.target.value)} placeholder='Enter Mobile Number' className='appearance-none  px-4 my-5 h-[50px] bg-[#F2F2F2] w-[88%] rounded-[7px] border border-black py-2' />
                                    <div className=" w-[88%]">
                                        <div className="w-[30%]">
                                            <Button name="Submit" style="px-8 w-full mx-auto flex justify-center" onclick={() => {
                                               getUpdates()
                                            }}
                            ...
                            <div key={i} className="box bg-[#F2F2F2] md:w-[45%] min-h-[300px]  my-4 px-2 md:px-8 p-4 rounded-[22px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]">
                                <h2 className='text-[#832729] py-2 md:text-[18px] text-[15px] font-[700]'>{`Tanishq Jewellery - ${data.storeName}`}</h2>
                                <p className='pb-4 font-[700] text-[#832729]  border-b-[#832729] border-b-[4px] cursor-pointer' onClick={() => Navigate(`/store-locator/jewellery-stores/${data.storeState.toLowerCase().replace(" ", "-")}/${data.storeCity.toLowerCase().replace(" ", "-")}/${data.storeName.toLowerCase().split("- ")[1] ? data.storeName.toLowerCase().split("- ")[1].replace(/ /g, "-").replace(",", "-") : data.storeName.toLowerCase().replace(" ", "-").replace(",", "-")}-${data.storeCode}`)}>Visit Store Details{">>>"}</p>
                    ...
                    {data.length <= 1 && <><div className="box bg-[#F2F2F2] md:w-[45%] min-h-[300px]  my-4 px-2 md:px-8 p-4 rounded-[22px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]">
                        <div className="flex justify-center items-center">
                            <div class="w-[40%] h-0.5 bg-neutral-700 rounded-md"></div>
                            <svg xmlns="http://www.w3.org/2000/svg" className='mx-3 mb-4' width="60" height="50" viewBox="0 0 60 50" fill="none">
                                <path d="M34.9499 6C34.9843 5.76589 35.0011 5.52926 34.9999 5.29234C34.9999 3.88872 34.4732 2.5426 33.5355 1.55009C32.5978 0.557584 31.326 0 30 0C28.6739 0 27.4021 0.557584 26.4645 1.55009C25.5268 2.5426 25 3.88872 25 5.29234C25.0001 5.52927 25.0173 5.76585 25.0514 6L26.4643 5.76411C26.4403 5.60814 26.4284 5.45036 26.4286 5.29234C26.4286 4.28976 26.8048 3.32824 27.4746 2.6193C28.1444 1.91037 29.0528 1.5121 30 1.5121C30.9472 1.5121 31.8556 1.91037 32.5253 2.6193C33.1951 3.32824 33.5714 4.28976 33.5714 5.29234C33.5713 5.45029 33.5599 5.60801 33.5371 5.76411L34.9499 6Z" fill="black" />
                        ...
                        <div className="flex justify-center items-center flex-col">
                            <h2 className=' text-center text-red-900 text-lg  font-medium md:w-[80%]'>Subscribe to get updates on best offers, gold rates & other benefits from Tanishq.</h2>
                            <input type="text" placeholder='Enter Mobile Number' onChange={(e)=>setmno(e.target.value)} className=' px-4 my-5 h-[50px] bg-[#F2F2F2] w-[88%] rounded-[7px] border border-black py-2' />
                            <div className=" w-[88%]">
                                <div className="w-[30%]">
                                    <Button name="Submit" style="px-8 w-full mx-auto flex justify-center" onclick={() => {
                                       getUpdates()
...
navigation={true}
                    ...
                    >
                        ...
                        <SwiperSlide><img src={crousal1} className='md:w-full w-[90%] max-h-[500px]' alt="" srcset="" /></SwiperSlide>
            ...
            <div className="flex md:hidden flex-col w-[90%] items-center justify-center ">
                <div className='my-10'>
                    ...
                    >
                        ...
                        <SwiperSlide><img src={MobBanner1} className='md:w-full w-[90%] max-h-[500px]' alt="" srcset="" /></SwiperSlide>
