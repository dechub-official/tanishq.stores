...
export default function BookAnAppointmentCity({ step, cities, activeStore, setIndividualStoreData, isOpen, setisOpen, days, weekDays, dayInfo, setDayInfo, handleDivisions, activeDivision, isTimeDisabled, timeDivisions, activeModal, setActiveModal, handleDate, handelStep, handleInputChange, title }) {
    return <>
        ...
        <div className={` block shadow-[#832729] shadow-[0_10px_20px_rgba(240,_46,_170,_0.7)] z-[90] absolute !overflow-visible  bottom-0 transition-all duration-150 md:hidden left-0 font-fraunces  h-fit  max-h-[490px] min-h-[490px] px-2  !rounded-t-xl  !rounded-b-none w-full bg-white`}>
            ...
            {step == 1 &&
                <>
                    <h1 className="text-black px-4 text-[22px] leading-normal font-fraunces  text-left mt-[2rem] mb-1 font-[400]">{title !== undefined ? <>{title}</> : <><span className="text-[#832729] font-fraunces ">Heading to </span> {cities[activeStore]?.storeName}</>}</h1>
                    <p className="text-black px-4 font-fraunces mb-5 mt-2 text-[14px]  font-normal">Let us make it smoother and tailored for you. <br />Book an appointment to skip the wait.</p>
                    <ButtonStripCity
                        days={days}
                        weekDays={weekDays}
                        dayInfo={dayInfo}
